...
class LSCommand : public testing::Test {
 public:
  void SetUp() override {}
...
Block sizes and directory totals are handled as described in The Long Format subsection below, except (if the long format is not also requested) the directory totals are not output when the output is in a single column, even if multi-column output is requested.  (-l) format, display complete time information for the file, including month, day, hour, minute, second, and year.  The -D option gives even more control over the output format.  This option is not defined in IEEE Std 1003.1-2008 (“POSIX.1”).)");
    ...
    // TODO:
    // parser.add_flag(",")
    //    .help(R"((Comma) When the -l option is set, print file sizes grouped
    //    and separated by thousands using the non-monetary separator returned
    //    by localeconv(3), typically a comma or period.  If no locale is set,
    //    or the locale does not have a non-monetary separator, this option has
    //    no effect.  This option is not defined in IEEE Std 1003.1-2001
    //    (“POSIX.1”).)");
...
class LSCommandBind : public testing::Test {
 public:
  void SetUp() override {}
  void TearDown() override {}
  LSCommandBind() {
...
TEST_F(LSCommandBind, lscmd_bind) {
...
TEST_F(LSCommandBind, parser3_bind) {
  ...
  ASSERT_EQ(option.files_or_directories[0], "./1");
  ASSERT_EQ(option.files_or_directories[1], "./1/2/3");
  ASSERT_EQ(option.files_or_directories[2], ".hided/");
}
