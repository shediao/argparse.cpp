...
argparse::ArgParser make_getopt_parser() {
    ...
    parser.add_flag("q,quiet")
        .help("Disable error reporting by getopt(3).");

    parser.add_flag("Q,quiet-output")
        .help("Do not generate normal output. Errors are still reported by getopt(3), unless you also use -q.");

    parser.add_option("s,shell")
        .help("Set quoting conventions to those of shell. If the -s option is not given, the BASH conventions are used. Valid arguments are currently 'sh' 'bash', 'csh', and 'tcsh'.");

    parser.add_flag("T,test")
        .help("Test if your getopt(1) is this enhanced version or an old version. This generates no output, and sets the error status to 4. Other implementations of getopt(1), and this version if the environment variable GETOPT_COMPATIBLE is set, will return '--' and error status 0.");

    parser.add_flag("u,unquoted")
        .help("Do not quote the output. Note that whitespace and special (shell-dependent) characters can cause havoc in this mode (like they do with other getopt(1) implementations).");

    parser.add_flag("V,version")
        .help("Display version information and exit. No other output is generated.");
...
TEST(getopt, parse){
  auto parser = make_getopt_parser();
  std::vector<const char*> cmd{"getopt", "-q", "-o", "hdrv", "-l", "help,debug,release,version", "--", "-a", "-b", "-c", "-d", "-e" };

  parser.parse(cmd.size(), cmd.data());

  ASSERT_TRUE(parser["quiet"].as<bool>());
  ASSERT_EQ("hdrv" ,parser["options"].as<std::string>());
  ASSERT_EQ("help,debug,release,version" ,parser["longoptions"].as<std::string>());
