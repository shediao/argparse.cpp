...
TEST(OptBase, Option_SetDefault) {
  ...
  ASSERT_EQ(str_opt.as<std::string>(), "");
  ASSERT_EQ(int_opt.as<int>(), 0);
  ASSERT_DOUBLE_EQ(double_opt.as<double>(), 0.0);
  ASSERT_EQ((pair_str_str_opt.as<std::pair<std::string, std::string>>()),
            (std::pair<std::string, std::string>{}));
  ASSERT_EQ((pair_str_int_opt.as<std::pair<std::string, int>>()),
            (std::pair<std::string, int>{}));

  ASSERT_TRUE((map_str_int_opt.as<std::map<std::string, int>>().empty()));
  ...
  ASSERT_EQ((map_str_int_opt.as<std::map<std::string, int>>().at("0000")), 0);
  ASSERT_EQ((map_str_int_opt.as<std::map<std::string, int>>().at("1111")), 1);

  ASSERT_EQ(str_opt.as<std::string>(), "11");
  ASSERT_EQ(int_opt.as<int>(), 13);
  ASSERT_DOUBLE_EQ(double_opt.as<double>(), 3.14);
  ASSERT_EQ((pair_str_str_opt.as<std::pair<std::string, std::string>>()),
  ...
  ASSERT_EQ(str_opt.as<std::string>(), "test");
  ASSERT_EQ(int_opt.as<int>(), 9);
  ASSERT_DOUBLE_EQ(double_opt.as<double>(), 2.71828);
  ASSERT_EQ((pair_str_str_opt.as<std::pair<std::string, std::string>>()),
            (std::pair<std::string, std::string>{"k2", "v2"}));
  ASSERT_EQ((pair_str_int_opt.as<std::pair<std::string, int>>()),
            (std::pair<std::string, int>{"192.168.1.1", 9999}));

  ASSERT_EQ((map_str_int_opt.as<std::map<std::string, int>>().at("e0")), 0);
  ASSERT_EQ((map_str_int_opt.as<std::map<std::string, int>>().at("e1")), 1);
...
TEST(OptBase, Positional_SetDefault) {
  ...
  auto const& positionals =
      str_int_positional.as<std::vector<std::pair<std::string, int>>>();
